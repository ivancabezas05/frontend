...
const BookCard = (props) => {
  ...
  useEffect(() => {
    const editorialLibro = editorialData.find((editorial) => editorial.id === libro.editorialid);
    if (editorialLibro) {
  ...
  return (
    <>
      <article className={styles.libro} key={isbn13}>
        <div className={styles.librocuadro1} key={isbn13}>
          <div className={styles.icono3}>
            <span>{obtenerSiglas(libro.titulo)}</span>
          </div>
          <p>{libro.titulo}</p>
        </div>
        <div className={styles.imglibro} key={isbn13}>
          <Link href={`../Citas/${isbn13}`} className={styles.libroContainer}>
        ...
        <div className={styles.informacionlibro} key={isbn13}>
